...
public class LockManager
{
    ...
    private void convertLock(DataObj dataObj,TrxnObj trxnObj){
	Vector vect = this.lockTable.elements(dataObj);
	DataObj dataObj2;
	int size = vect.size();
	...
	dataObj2 = (DataObj) vect.elementAt(i);
		if (dataObj.getXId() == dataObj2.getXId()) {
			if (dataObj.getLockType() == DataObj.WRITE) {
	...
	this.lockTable.remove((DataObj) vect.elementAt(i));
	...
	trxnObj2 = (TrxnObj) vect.elementAt(i);
		if (trxnObj.getXId() == trxnObj2.getXId()) {
			if (trxnObj.getLockType() == trxnObj.WRITE) {
		    		if((trxnObj2.getLockType() == trxnObj.READ) && (trxnObj2.getDataName().equalsIgnoreCase(trxnObj.getDataName()))) {
		    			break;	
		    		}
		    	}
		}
	...
	this.lockTable.remove((TrxnObj)vect.elementAt(i));
